...
import { User, Lock, Mail, X, UserPlus, Loader2, AlertCircle, ArrowLeft, KeyRound, CheckCircle2 } from 'lucide-react';
...
const AuthModal: React.FC<AuthModalProps> = ({ onLogin, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] bg-slate-950/90 backdrop-blur-md p-4 animate-in fade-in duration-200 overflow-y-auto">
            ...
            <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden relative">
                ...
                {view === 'LOGIN' && (
                    <div className="p-8">
                        ...
                        >
                            {githubLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><GithubIcon className="w-5 h-5 mr-2" /> Continue with GitHub</>}
                        </button>
                ...
                {view === 'REGISTER' && (
                    <div className="p-8">
                        ...
                        <button 
                            ...
                            className="w-full bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold py-3 rounded-lg transition-all flex items-center justify-center border border-slate-700"
                        >
                            {githubLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><GithubIcon className="w-5 h-5 mr-2" /> Sign up with GitHub</>}
                        </button>
